...
class GridWorldAgent:
    ...
    def define_position_propositions(self, p_name, cell):
        ...
        BUT this should NOT automatically trigger task operations.
        """
        i, j = cell
        p_vars = {}
        ...
        # Link p_vars[t] == b[t,i,j] - this just tracks physical presence
        for t in range(self.T+1):
            self.model.addConstr(
    ...
    def solve(self):
        ...
        """
        self.model.Params.OutputFlag = 0  # don't show Gurobi logs
        self.model.optimize()
    ...
    def stl_until(self, p_vars, q_vars, name="UNTIL"):
        ...
        # Also, p_vars[k] must be 1 for k in [t..tp-1]
        for t in range(T+1):
            for tp in range(t, T+1):
    ...
    def stl_implies_next(self, left_vars, right_vars, name="IMPLIES_NEXT", task_id=None):
        """
        IMPLIES_NEXT with separate task operation variables.
        Physical presence ≠ Task operation.
        """
        model = self.model
        T = self.T

        # Create separate task operation variables
        pickup_operation = {}
        dropoff_operation = {}
        
        for t in range(T+1):
            pickup_operation[t] = model.addVar(vtype=GRB.BINARY, name=f"{name}_pickup_op_{t}")
            dropoff_operation[t] = model.addVar(vtype=GRB.BINARY, name=f"{name}_dropoff_op_{t}")
        model.update()

        # Task operations can only happen when physically present
        for t in range(T+1):
            model.addConstr(pickup_operation[t] <= left_vars[t], 
                        name=f"{name}_pickup_requires_presence_{t}")
            model.addConstr(dropoff_operation[t] <= right_vars[t],
                        name=f"{name}_dropoff_requires_presence_{t}")

        # Exactly one pickup operation, exactly one dropoff operation
        model.addConstr(gp.quicksum(pickup_operation[t] for t in range(T+1)) == 1, 
                    name=f"{name}_exactly_one_pickup_op")
        model.addConstr(gp.quicksum(dropoff_operation[t] for t in range(T+1)) == 1,
                    name=f"{name}_exactly_one_dropoff_op")

        # Precedence: no dropoff operation before pickup operation
        for t in range(T+1):
            model.addConstr(
                dropoff_operation[t] <= gp.quicksum(pickup_operation[tau] for tau in range(t+1)),
                name=f"{name}_no_dropoff_before_pickup_{t}"
            )

        # Create phi[t] for this specific task
        phi = {}
        for t in range(T+1):
            phi[t] = model.addVar(vtype=GRB.BINARY, name=f"{name}_phi_{t}")
        model.update()

        # Task state tracking based on operations, not just presence
        task_started = {}
        task_completed = {}
        task_active = {}
        
        for t in range(T+1):
            task_started[t] = model.addVar(vtype=GRB.BINARY, name=f"{name}_started_{t}")
            task_completed[t] = model.addVar(vtype=GRB.BINARY, name=f"{name}_completed_{t}")
            task_active[t] = model.addVar(vtype=GRB.BINARY, name=f"{name}_active_{t}")
        model.update()

        # Task starts when pickup operation happens
        model.addConstr(task_started[0] == pickup_operation[0])
        for t in range(1, T+1):
            model.addConstr(task_started[t] >= task_started[t-1])
            model.addConstr(task_started[t] >= pickup_operation[t])
            model.addConstr(task_started[t] <= task_started[t-1] + pickup_operation[t])

        # Task completes when dropoff operation happens  
        model.addConstr(task_completed[0] == 0)
        for t in range(1, T+1):
            model.addConstr(task_completed[t] >= task_completed[t-1])
            model.addConstr(task_completed[t] >= task_completed[t-1] + dropoff_operation[t])
            model.addConstr(task_completed[t] <= task_completed[t-1] + dropoff_operation[t])

        # Task is active if started but not completed
        for t in range(T+1):
            model.addConstr(task_active[t] <= task_started[t])
            model.addConstr(task_active[t] <= 1 - task_completed[t])
            model.addConstr(task_active[t] >= task_started[t] - task_completed[t])

        # Store task state using OPERATION variables, not presence variables
        if task_id is not None:
            ...
            self._task_states[task_id] = {
                ...
                'left_vars': pickup_operation,    # Use operation vars, not presence vars
                'right_vars': dropoff_operation,  # Use operation vars, not presence vars
                'pickup_locations': left_vars,    # Keep reference to location presence
                'dropoff_locations': right_vars   # Keep reference to location presence
            }

        # phi[t] = 1 when task constraint is satisfied
        for t in range(T+1):
            model.addConstr(phi[t] == 1 - task_active[t])

        return phi
    # def add_inter_task_constraints(self):
    #     """
    #     Minimal constraints: prevent only logical violations, allow optimal scheduling.
    #     """
    #     if not hasattr(self, '_task_states') or len(self._task_states) < 2:
    #         return

    #     model = self.model
    #     T = self.T
    #     task_ids = list(self._task_states.keys())

    #     # ONLY constraint: at most one pickup operation at any timestep
    #     # This prevents simultaneous pickups but allows optimal ordering
    #     for t in range(T+1):
    #         pickup_operations = [self._task_states[tid]['left_vars'][t] for tid in task_ids]
    #         if pickup_operations:
    #             model.addConstr(gp.quicksum(pickup_operations) <= 1, 
    #                         name=f"single_pickup_op_{t}")
    
    def add_inter_task_constraints(self):
        """
        Enforce proper task completion: no new pickup while any task is active.
        """
        if not hasattr(self, '_task_states') or len(self._task_states) < 2:
            return
        ...
        # Core constraint: at most one task can be active at any time
        for t in range(T+1):
            active_tasks = [self._task_states[tid]['active'][t] for tid in task_ids]
            model.addConstr(gp.quicksum(active_tasks) <= 1, 
                        name=f"single_active_task_{t}")
        
        # # Redundant but helpful constraint: at most one pickup operation at any time
        # for t in range(T+1):
        #     pickup_operations = [self._task_states[tid]['left_vars'][t] for tid in task_ids]
        #     if pickup_operations:
        #         model.addConstr(gp.quicksum(pickup_operations) <= 1, 
    ...
    # def add_inter_task_constraints(self):
    #     """
    #     Flexible but correct: prevent overlapping active tasks.
    #     Allow efficient task scheduling while maintaining logical consistency.
    #     """
    #     if not hasattr(self, '_task_states') or len(self._task_states) < 2:
    #         return
    ...
    #     # Additional constraint: at most one pickup operation at any time
    #     for t in range(T+1):
    #         pickup_operations = [self._task_states[tid]['left_vars'][t] for tid in task_ids]
    #         model.addConstr(gp.quicksum(pickup_operations) <= 1, 
    #                     name=f"single_pickup_op_{t}")
    def encode_stl(self, formula, top_level = True):
        ...
        if op == "F":
            ...
            return self.stl_implies_next(left_sub, right_sub, task_id=task_id)
        elif op == "FIRST":
    ...
    # (sum_help_chosen >= 1). If we want to ensure it *must* be visited:
        ...
        self.add_inter_task_constraints()

        if hasattr(self, '_task_states') and self._task_states:
            # Simpler approach: T_all >= max completion time
            T_all = self.model.addVar(vtype=GRB.INTEGER, lb=0, ub=self.T, name="T_all_tasks")
            
            for task_id, task_state in self._task_states.items():
                # For each task, ensure T_all is at least when it completes
                for t in range(self.T + 1):
                    # Only add constraint when task actually completes (not for persistent completion)
                    if t == 0:
                        # First timestep
                        self.model.addConstr(T_all >= t * task_state['completed'][t])
                    else:
                        # Only count the transition from not-completed to completed
                        task_completes_now = self.model.addVar(vtype=GRB.BINARY, name=f"{task_id}_completes_{t}")
                        self.model.addConstr(task_completes_now >= task_state['completed'][t] - task_state['completed'][t-1])
                        self.model.addConstr(task_completes_now <= task_state['completed'][t])
                        self.model.addConstr(task_completes_now <= 1 - task_state['completed'][t-1])
                        
                        self.model.addConstr(T_all >= t * task_completes_now)
            
            self.T_all = T_all
            
            # For compatibility, create a dummy all_visited
            self.all_visited = {t: self.model.addVar(vtype=GRB.BINARY) for t in range(self.T+1)}
            for t in range(self.T+1):
                # all_visited[t] = 1 when all tasks completed by time t
                all_tasks_done = []
                for task_state in self._task_states.values():
                    all_tasks_done.append(task_state['completed'][t])
                
                if all_tasks_done:
                    self.model.addConstr(self.all_visited[t] <= gp.quicksum(all_tasks_done) / len(all_tasks_done))
                    self.model.addConstr(self.all_visited[t] >= gp.quicksum(all_tasks_done) - len(all_tasks_done) + 1)
        else:
            # Fallback to old logic if no tasks
        ...
        for t in range(self.T):
            abs_dx = self.model.addVar(vtype=GRB.CONTINUOUS, lb=0, name=f"abs_dx_{t}")
            abs_dy = self.model.addVar(vtype=GRB.CONTINUOUS, lb=0, name=f"abs_dy_{t}")
            
            self.model.addConstr(abs_dx >= self.dx[t])
            self.model.addConstr(abs_dx >= -self.dx[t])
            self.model.addConstr(abs_dy >= self.dy[t])
            self.model.addConstr(abs_dy >= -self.dy[t])
            
            dist_expr += abs_dx + abs_dy
    ...
    def reset_and_rebuild_model(self):
        ...
        # CRITICAL: Clear task states to avoid stale variable references
        if hasattr(self, '_task_states'):
            self._task_states.clear()
        if hasattr(self, '_task_counter'):
            self._task_counter = 0
